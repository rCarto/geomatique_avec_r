...
Pour construire un model, nous utilisons dans un premier temps la fonction `project(x, crs)`, puis la fonction `res()` (ou `dim()`) pour définir la résolution de ce nouveau raster (voir `?project`). La fonction `project()` peut ensuite être réutiliser pour projeter les valeurs dans le nouveau raster, en précisant la méthode d'estimation à utiliser. 
