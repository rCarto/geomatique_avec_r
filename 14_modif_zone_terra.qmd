...
Pour construire un model vous pouvez d'abord faire `project(x, crs)` et ensuite manipuler la sortie pour obtenir le modèle que vous voulez. Par exemple, dans la mesure du possible, utilisez des nombres entiers pour l'étendue et la résolution. 
Vous pouvez utiliser des commandes telles que `dim(z) = c(180, 360)` ou `res(z) <- 100000`. (voir `?project`)
