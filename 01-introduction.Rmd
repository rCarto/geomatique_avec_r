...
- Le package `raster` [@R-raster] est dédié à l'import, la manipulation et la modélisation des données raster.   

Aujourd'hui, les principaux développements concernant les données vectorielles se sont détachés des 3 anciens (`sp`, `rgdal`, `rgeos`) pour se reposer principalement sur le package `sf` ([@R-sf], [@Pebesma18]). Dans ce manuel nous nous appuierons exclusivement sur ce package pour manipuler les données vectorielles. 
