...
Sélection des carreaux de la grille qui intersectent le département (`st_filter()`).

```{r intersect3.1, nm=TRUE}
grid <- st_filter(grid, dep_46)
...
Dans cet exemple nous importons un fichier csv (**pop.csv**) qui contient les
données d’une grille de population (x et y longitude et latitude, EPSG:3035; Ind, la population).
...
```{r agval1.1, nm=TRUE}
plot(st_geometry(com))
plot(st_geometry(pop), bg = "red", pch = 21, cex = .5, add = TRUE)
```



En utilisant la fonction `st_join()` nous rajoutons à chacun des point de la grille le code de la commune dans lequel il se trouve. 

```{r agval1.33}
# intersection
inter <- st_join(pop, com[,"INSEE_COM"])
head(inter, 10)
```
...
```{r exo1_2, nm=TRUE, eval=TRUE, echo = TRUE}
